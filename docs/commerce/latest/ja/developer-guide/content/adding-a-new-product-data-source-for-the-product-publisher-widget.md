...
1. サンプルの商品データソースが追加されたことを確認します。 ブラウザーで  `https://localhost:8080`  を開き、商品パブリッシャーウィジェットのあるページに移動します。 商品パブリッシャーの［_設定_］をクリックし、［_製品の選択_］セクションで［_データソース_］を選択します。 新しい商品データソース（「Products Ending in the Same Word」）が、［_データソース_］ ドロップダウンの下に表示されます。
