...
1. サンプルの割引ルールタイプが追加されたことを確認します。 ブラウザで`https://localhost:8080`を開きます。 次に、アプリケーションメニュー（![Applications Menu](../../images/icon-applications-menu.png)）をクリックし、［_コマース_］→［_割引_］に移動します。 メニュー内の［_編集_］をクリックして、割引を適用します。 ［_ルール_］セクションまでスクロールし、(+)アイコンをクリックして、新しい割引ルールを追加します。 ［_タイプ_］ ドロップダウンの下に、新しい割引ルールタイプ（「Has a minimum number of products」）が表示されます。
