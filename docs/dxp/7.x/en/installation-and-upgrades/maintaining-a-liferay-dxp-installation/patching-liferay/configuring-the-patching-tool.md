...
| :------- | :---------- |
| `global.lib.path` | Specifies the location for storing `.jar` files on the global classpath. Hint: `portal-kernel.jar` is on the global classpath. In DXP 7.2, `global.lib.path` is only used if the `patching.mode` is `binary`.  |
