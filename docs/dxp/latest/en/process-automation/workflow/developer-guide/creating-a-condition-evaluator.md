...
Workflow [condition nodes](../designing-and-managing-workflows/workflow-designer/using-condition-nodes.md) use a Groovy script to determine the proper transition a workflow item traverses. Instead of writing logic in Groovy directly in a workflow definition `<script>` element, you can execute Java logic by implementing the `ConditionEvaluator` interface. 
...
In addition to the condition evaluator the project includes and autoloads a workflow definition called the R6J9 Conditional Approver, which uses the condition evaluator.
