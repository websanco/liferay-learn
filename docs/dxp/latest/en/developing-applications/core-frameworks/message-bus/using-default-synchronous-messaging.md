...
The three example modules provide classes that manage destinations, listen for messages, and send messages.
...
Examine each class, starting with the destination configurators.
...
The `m4q7-baker-impl` module's `M4Q7BakerOSGiCommands` class below provides an OSGi Command that triggers sending the command arguments in a message to the `"acme/m4q7_able"` destination.
...
Turn off the message response logic following these steps.
...
1. Confirm the Gogo shell page looks like the figure below.
