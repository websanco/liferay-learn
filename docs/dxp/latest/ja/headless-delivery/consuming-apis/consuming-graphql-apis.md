...
1. GraphQLが分離読み取りおよび書き込み動作第1のA呼び出すことにより、 **クエリ** 及び第 **の突然変異** 。 最初に行うのはブログエントリの投稿なので、 **ミューテーション** クリックします。

1. API全体のリストが表示されます。上部の検索を使用するか下にスクロールして、 `createSiteBlogPosting`への呼び出しを見つけます。
