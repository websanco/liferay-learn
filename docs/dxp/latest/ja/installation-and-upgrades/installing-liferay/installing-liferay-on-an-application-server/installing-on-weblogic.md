...
DXPには、デモンストレーション目的で組み込みのHypersonicデータベースが含まれていますが、 _は本番環境では使用しないでください_。 フル機能のサポートされているデータベースを使用します。 データベースのセットアップについては、[Configure a Database](../configuring-a-database.md)を参照してください。
