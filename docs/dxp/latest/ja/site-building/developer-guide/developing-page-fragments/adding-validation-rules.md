...
``` note::
   正規表現値の文字をエスケープするためにバックスラッシュを使用する場合（例：``\d`）、最初にバックスラッシュをエスケープするために``JSON``ファイル内で2つのバックスラッシュを使用する必要があります (例：``\\d``)。 JSONの文法の詳細は、 `IETF <./https://www.ietf.org/rfc/rfc4627.txt>`_ ドキュメンテーションを参照してください。
```
